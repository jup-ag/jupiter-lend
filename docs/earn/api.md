...
async function main() {
  ...
  try {
    ...
    // Option 1: Using complete transaction
    const depositResponse = await axios.post(
      "https://lend-api.jup.ag/v1/lending/deposit",
      depositPayload
    );
    const depositTx = Transaction.from(
      Buffer.from(depositResponse.data.transaction, "base64")
    );
    const signedDepositTx = await window.solana.signTransaction(depositTx);
    const depositTxid = await connection.sendRawTransaction(
    ...
    const withdrawInstruction = new TransactionInstruction({
      programId: new PublicKey(withdrawResponse.data.instructions[0].programId),
      keys: withdrawResponse.data.instructions[0].accounts.map((account) => ({
        ...account,
        pubkey: new PublicKey(account.pubkey),
      })),
      data: Buffer.from(withdrawResponse.data.instructions[0].data, "base64"),
    });
    ...
    const signedWithdrawTx = await window.solana.signTransaction(withdrawTx);
    const withdrawTxid = await connection.sendRawTransaction(
